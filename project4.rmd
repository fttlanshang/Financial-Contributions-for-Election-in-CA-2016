...
Since contribution time was considered as a factor variable, I needed to transform it to date format first. Here, I simply replaced the variable with Date format. Also, I created another column in this dataset, which only recorded the year and month of the contribution, which was used for later exploration and analysis.

When looking at the summary or plot, we could find that the earliest contribution started on 2013. Actually, we could see from the bar chart that most contributions were made in 2016. And there are two peaks in 2016, one in 201603 (during primary election), and the other in 201610 (one month before the general election).
...
When using whole dataset(containing negative contribution receipt amount), all three types appeared. But when using positive subset(which was used through the exploration), SA28A type disappeared. But actually, I once extracted the negative subset and looked at its summary data, all 3 types were concluded, not only SA28A as I expected.
...
- I changed the format of the contribution date and also creates another column(called `contb_receipt_dt_yr_mo`) representing the year and month of the contribution.
- I combined the committee id and candidate name to form a new column to check whether committees and candidates were correlated one by one.
- I created another two variables, i.e., top 10 cities and top 10 occupations ranked by contribution times since there were too many levels for cities and occupations.
...
ggplot(aes(x = contbr_city), data = subset(financial, 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
# Maybe it's a good idea to draw the three plots into one, but that way, I can't change the scale of y.
```
...
It was very similar to the bivariate one talked above, except changing from count to financial receipt amount. So here I still only cared about three candidates, Clinton, Hillary Rodham, Sanders, Bernard and Trump, Donald J. 
...
ggplot(aes(x = cand_nm, y = total_amt), 
  ...
  facet_wrap(~election_tp, scales = "free_y") + labs(x = "candidate name", y = "financial contribution amount",
      title = "Financial Contribution Amount for Each Candidate During Different Election Period") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
For future analysis, to better intepret the data, it would be very important to combine the data with reality. And it's one drawback in my current analysis.
